...
export async function getProducts(limit?: number) {
  ...
  try {
    return await searchRead(
      ...
      ['name', 'display_name', 'barcode', 'list_price', 'standard_price', 'qty_available', 'fmcg_reorder_threshold', 'fmcg_is_low_stock', 'image_512', 'product_tmpl_id'],
      limit
    );
  } catch {
    return await searchRead(
      'product.product',
      [['active', '=', true], ['type', '=', 'consu']],
      ['name', 'display_name', 'barcode', 'list_price', 'standard_price', 'qty_available', 'image_512', 'product_tmpl_id'],
      limit
    );
  }
}
